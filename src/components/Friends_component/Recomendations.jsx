    ...
    <div className="bg-gray-50 bg-opacity-50 w-full rounded-lg shadow-lg p-6">
      ...
      <ul className="flex flex-wrap items-center justify-center gap-x-5 gap-y-2">
        {movies.slice(0, 6).map(movie => (
          <li key={movie.title} className="flex flex-col items-center bg-white rounded-lg hover:shadow-lg p-4 w-28 md:w-40">
            <a href={`https://letterboxd.com${movie.url}`} target="_blank" rel="noopener noreferrer" className="text-center">
              <img src={movie.posterUrl} alt={`${movie.title} poster`} className=" w-20 h-28 md:w-32 md:h-48 object-cover rounded-sm mb-2" />
