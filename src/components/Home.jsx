...
function Home() {
    ...
    return (
        <div className='h-[100vh] w-auto relative flex flex-col md:gap-15 gap-8 justify-center items-center overflow-hidden m-auto px-10'>
            ...
            <div className='flex z-20 w-[80%] md:w-full max-w-md mx-auto'>
                <button className={`font-sans py-2 md:py-2 flex-1 text-white rounded-l-full border-r transition-colors duration-100 ease-in-out ${
                    navOption === 'stats' ? 'bg-blue-600' : 'bg-gray-500'
                }`} onClick={() => setnavOption('stats')}> Stats </button>
                <button className={`font-sans py-2 md:py-2 flex-1 text-white text-md trac transition-colors duration-100 ease-in-out ${
                    navOption === 'reviews' ? 'bg-blue-600' : 'bg-gray-500'
                }`} onClick={() => setnavOption('reviews')}> Reviews</button>
                <button className={`font-sans py-2 md:py-2 flex-1 text-white rounded-r-full border-l transition-colors duration-100 ease-in-out ${
                    navOption === 'friends' ? 'bg-blue-600' : 'bg-gray-500'
            ...
            <form onSubmit={handleSearch} className='z-20 flex flex-col items-center w-full max-w-md'>
                <input 
                    type="text" 
                    className='placeholder:italic placeholder:text-sm w-[80%] md:w-full border border-slate-700 p-2 md:p-3 block rounded-2xl focus:outline-none focus:border-gray-500 focus:ring-gray-500 focus:ring-1 text-xl font-sans'
                    value={username}
